...
export class CircuitRenderChild extends MarkdownRenderChild {
	...
	private getCircuitJsUrl(): string {
		if (this.settings.offlineMode && Platform.isDesktopApp) {
			...
			if (adapter instanceof FileSystemAdapter) {
				...
				// Plugin assets are in {configDir}/plugins/obsidian-circuitjs/circuitjs/
				const pluginPath = `${basePath}/${configDir}/plugins/obsidian-circuitjs/circuitjs/circuitjs.html`;
	...
	private createWebview(container: HTMLElement): void {
		...
		// Set inline styles to fix webview height collapse
		// The internal iframe has flex: 1 1 auto which collapses without explicit parent sizing
		webviewNode.style.height = "600px";
		webviewNode.style.minHeight = "600px";
		webviewNode.style.flex = "none";
		webviewNode.style.display = "block";
